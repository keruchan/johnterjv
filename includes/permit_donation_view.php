...
function render_permit_donation_requirement(
    ...
    <div class="border rounded p-3">
        ...
        <div class="alert alert-warning mt-3 mb-0" role="note"><i class="bi bi-exclamation-triangle me-1"></i><?php if ((string) $requirement['current_status'] === 'ems_verified'): ?>EMS verification complete — final RPS confirmation and release are still required.<?php elseif ((string) $requirement['current_status'] === 'rps_verified'): ?>Donation verified — permit release is a separate step, not yet done.<?php else: ?>Not release-ready yet — EMS receipt/verification and final RPS confirmation are still needed.<?php endif; ?></div>
    </div>
    <?php
}
